...
const NewFeatureCard = ({ icon, title, description, link, color }) => (
    ...
    >
        <span className="text-3xl md:text-4xl mb-4" role="img" aria-label={title}>{icon}</span>
        <h3 id={`card-title-${title.toLowerCase().replace(/\s+/g, '-')}`} className="text-xl md:text-2xl font-bold mb-2">{title}</h3>
        <p className="text-gray-200 text-sm md:text-base flex-grow">{description}</p>
    </Link>
);
...
const PatientDashboard = () => {
  ...
  return (
    <main className="bg-amber-200 dark:bg-primary-dark min-h-full pt-20 md:pt-24 pb-8 md:pb-12 px-4">
      <div className="container mx-auto">
        ...
        <section aria-labelledby="care-navigator-title" className="mb-8 md:mb-12 bg-secondary-dark text-center py-8 md:py-12 px-4 rounded-lg">
          ...
          <Link
            to="/care-navigator"
            className="bg-accent-blue hover:bg-accent-blue-hover text-white font-bold py-2 md:py-3 px-6 md:px-8 rounded-lg text-base md:text-lg transition-transform transform hover:scale-105 inline-block focus:ring-2 focus:ring-offset-2 focus:ring-accent-blue focus:outline-none"
            role="button"
        ...
        <section aria-label="Healthcare Services" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
          <NewFeatureCard 
            icon="🩺"
            title="Find a Doctor"
